/**
 * One-shot: rewrite FCG header/footer/scripts on the 3 cloned sister
 * pages under /mechanical/ to use FCM chrome instead.
 *
 * Run once: node scripts/rewrite-mechanical-chrome.js
 */
const fs = require("fs");
...
const DROPDOWN_CSS = `
    ...
    @media (max-width: 1023px) {
      ...
      .site-nav__dropdown.is-open { max-height: 400px; }
      .site-nav__dropdown-link {
        padding-block: var(--space-3); padding-left: var(--space-5);
        border-bottom: 1px solid rgba(74, 125, 82, 0.25);
      }
    }
`;
...
const FCM_FOOTER_COLUMNS = `<div>
          ...
          <ul class="site-footer__list">
            ...
            <li><a href="/dfw">Dallas-Fort Worth, TX</a></li>
            <li><a href="/central-texas">Austin, TX</a></li>
          </ul>
        </div>
        <div>
...
function process(file) {
  console.log("Processing", path.relative(ROOT, file));
  let html = fs.readFileSync(file, "utf8");
  const before = html.length;

  // 1. Replace header (logo + nav). Anchor on the FCG header-logo opening tag
  //    through whatever the page's CTA button is (varies: "Get in Touch", "Open Positions", etc.).
  //    We just preserve whatever CTA href + label the original file had.
  html = html.replace(
    /<a class="site-header__logo"[^>]*>[\s\S]*?<a class="btn btn--primary btn--sm hide-mobile" href="([^"]+)">([^<]+)<\/a>/,
    (_, ctaHref, ctaLabel) => FCM_NAV_BLOCK.replace(
      '<a class="btn btn--primary btn--sm hide-mobile" href="/contact">Get in Touch</a>',
      `<a class="btn btn--primary btn--sm hide-mobile" href="${ctaHref}">${ctaLabel}</a>`
    )
  );

  // 2. Replace footer brand SVG (FCG -> FCM)
  html = html.replace(
    /<svg xmlns="http:\/\/www\.w3\.org\/2000\/svg" viewBox="0 0 1080 339\.464" aria-label="FirstCall Group">[\s\S]*?<\/svg>/,
    FCM_FOOTER_BRAND_SVG
  );

  // 3. Update footer brand paragraph
  html = html.replace(
  ...
  // 6. Inject dropdown CSS before </style>
  html = html.replace("  </style>", DROPDOWN_CSS + "  </style>");

  // 7. Inject dropdown JS after initMobileNav function
  html = html.replace(
    /(function initMobileNav\(\) \{[\s\S]*?\n      \})/,
    "$1" + DROPDOWN_JS
  );

  // 8. Call initDropdowns alongside initMobileNav
  html = html.replace(/initMobileNav\(\);/g, "initMobileNav();\n          initDropdowns();");

  // 9. Fix script paths
  html = html.replace(/src="assets\/js\//g, 'src="/assets/js/');

  // 10. Make internal links root-relative
  const linkMap = {
  ...
  console.log(`  ${before} -> ${html.length} bytes`);
}

for (const t of TARGETS) process(t);
console.log("Done.");
